                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover table-striped table-vcenter mb-0 text-nowrap">
                            ...
                            <tr>
                                <th colspan="5">{{__('tables.total_commands')}} : {{$cities->count()}}</th>
                            ...
                            @foreach($cities as $city)
                                <tr>
                                    ...
                                    <td><span>{{$city->pay}}</span></td>

                                    <td><a wire:click="editCity({{ $city->id }})" href="javascript:void(0);"
                                           class="btn btn-success btn-sm" onclick="topFunction()">
                                            {{__('action.edit')}}
                                        </a>
                                    </td>
                                    <td>
                                        <button wire:click="deleteCity({{$city->id}})" class="btn btn-danger btn-sm">
