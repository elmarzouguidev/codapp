                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover table-striped table-vcenter mb-0 text-nowrap">
                            ...
                            <tr>
                                @if($isUpdate)
                                    @include('livewire.permission.__update')
                                @endif
                                @if($isCreate)
                                    @include('livewire.permission.__create')
                                @endif
                            </tr>
                            <tr>
                                <th class="w30">&nbsp;</th>
                            ...
                            @foreach($permissions as $permission)
                                <tr>
                                    ...
                                    <td><a wire:click="editPermission({{ $permission->id }})" href="javascript:void(0);"
                                           class="btn btn-success btn-sm" onclick="topFunction()">
                                            {{__('action.edit')}}
                                        </a>
                                    </td>
                                    <td>
                                        <button wire:click="deletePermission({{$permission->id}})" class="btn btn-danger btn-sm">
